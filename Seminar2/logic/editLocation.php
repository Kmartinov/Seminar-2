...
if ($validated_data !== FALSE) {
	...
	// ako kategorija nije mjenjana, preskočit će provjeru u bazi
	if($validated_data['catId']!=0){
		...
		if(!$catIds)
			$validated_data = NULL;
	} else $validated_data['catId'] = false;
			...
			//Determine the path to which we want to save this file
			  ...
			  if (!file_exists($newname)) {
				...
				if ((move_uploaded_file($_FILES['pageImg']['tmp_name'],$newname))) {
				   ...
				   $validated_data = NULL;
				   $uploadError = true;
				   $greska = "Neuspjelo dodavanje slike. Molim pokušajte ponovo.";
				}
			  } else {
				$validated_data  = NULL;
				$uploadError = true;
				$greska = "Greška: Slika ".$_FILES["pageImg"]["name"]." već postoji";
			  }
		  } else {
			$validated_data = NULL;
			$uploadError = true;
			$greska = "Greška: Slika mora biti jpeg/gif, veličine do 1 MB";
		  }
	} else $validated_data['pageImg'] = false;
}
