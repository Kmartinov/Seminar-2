...
class GUMP
{
	...
	*/
	...
	protected function validate_alpha($field, $input, $param = NULL)
	{
		...
		if(!preg_match("/^([a-zŠĐČĆŽÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝšđčćžàáâãäåçèéêëìíîïðòóôõöùúûüýÿ])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
	...
	protected function validate_alpha_numeric($field, $input, $param = NULL)
	{
		...
		if(!preg_match("/^([a-z0-9ŠĐČĆŽÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝšđčćžàáâãäåçèéêëìíîïðòóôõöùúûüýÿ])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
	...
	protected function validate_alpha_dash($field, $input, $param = NULL)
	{
		...
		if(!preg_match("/^([a-z0-9ŠĐČĆŽÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝšđčćžàáâãäåçèéêëìíîïðòóôõöùúûüýÿ_-])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
	...
	protected function validate_street_address($field, $input, $param = NULL)
	{
		...
		$hasLetter = preg_match('/[a-zA-ZŠĐČĆŽÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝšđčćžàáâãäåçèéêëìíîïðòóôõöùúûüýÿ]/', $input[$field]);
